...
# 1. Le dossier à analyser ('.' signifie le dossier courant, là où est le script)
DOSSIER_CIBLE = '..'

# 2. Ton nouveau template de header complet
# (Mets bien les triples guillemets """ pour gérer le texte sur plusieurs lignes)
...
def update_headers():
    ...
    for nom_fichier in os.listdir(DOSSIER_CIBLE):
        ...
        if nom_fichier.endswith('.html'):
            ...
            # La Regex magique : 
            # <header.*?</header> cherche tout ce qui commence par <header et finit par </header>
            # re.DOTALL permet au '.' de correspondre aussi aux sauts de ligne (\n)
            pattern = re.compile(r'<header.*?</header>', re.DOTALL)

            # Si on trouve une balise <header> dans le fichier
            if pattern.search(contenu):
                # On la remplace par le nouveau template
                ...
                print(f"✅ Header mis à jour avec succès dans : {nom_fichier}")
                fichiers_modifies += 1

    print(f"\n🚀 Terminé ! {fichiers_modifies} fichiers ont été mis à jour.")
